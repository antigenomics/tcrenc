...
# Добавьте это в начало main() в run.py
...
def main():
    ...
    parser.add_argument('--embed_type', type=str, required=True, choices=['onehot', 'kidera'], 
                       help='Type of embedding: onehot or kidera')
    parser.add_argument('--residual_block', type=str, default='false',
                   choices=['true', 'false'],
    ...
    if script_name=='./code/code_kidera/process_kidera.py':
        command = [
            ...
            '--input', args.input,
            '--output', args.output,
            '--residual_block', str(residual_block)
        ]
    else:
        command = [
            'python',
